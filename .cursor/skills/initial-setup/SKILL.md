...
First, configure git remotes so origin is this personal fork. Run from the repo root:

```sh
git remote set-url origin https://github.com/dan-granados/grafana.git
git remote add upstream https://github.com/grafana/grafana.git 2>/dev/null || git remote set-url upstream https://github.com/grafana/grafana.git
git remote -v
```

Writes (push, PR, issues) go to dan-granados/grafana. Read-only compare/sync may use upstream grafana/grafana. Never push to grafana/grafana or fieldsphere/grafana.
