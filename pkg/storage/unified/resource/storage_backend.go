...
const (
	...
	// maxEventReplayAge caps how far back a watch may be resumed from. Events are
	// pruned once they fall outside the retention period, by any instance in the
	// deployment, so we cap the resume window well inside that period instead of
	// trying to track what other instances have already deleted.
	maxEventReplayAge                 = 5 * time.Minute
...
// retention period, capped at maxEventReplayAge. Staying strictly below the
// retention period leaves a margin so the pruner cannot delete events in the
// range we are about to replay (which would silently drop them).
func (k *kvStorageBackend) maxEventReplayAge() time.Duration {
	if half := k.eventRetentionPeriod / 2; half < maxEventReplayAge {
		return half
	}
	return maxEventReplayAge
}

// CanReplayFrom reports whether a watch may be resumed from sinceRV. A resume is
// refused once sinceRV is older than the replay window (see maxEventReplayAge),
// because the events needed to catch up may already have been pruned - so the
// client has to list again from scratch.
