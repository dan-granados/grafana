...
func (s *server) Watch(req *resourcepb.WatchRequest, srv resourcepb.ResourceStore_WatchServer) (retErr error) {
	...
	// sendEvent forwards a single write event to the client, applying the query
	// key and authz filters. It is used both for events replayed from the event
	// store and for live events coming from the broadcaster.
