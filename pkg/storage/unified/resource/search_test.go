...
// The case the requirement exists for: an index keeping deleted documents must not
// be read by an instance that would return them as live.
func TestIndexReaderRequirementsDeclaresDeletedMarker(t *testing.T) {
	require.Contains(t, IndexReaderRequirements(), IndexFeatureDeletedMarker)

	// Incomplete results are safe to serve, so these are not requirements.
	require.NotContains(t, IndexReaderRequirements(), IndexFeatureStoredFacets)
	require.NotContains(t, IndexReaderRequirements(), IndexFeatureTrashFields)
}
